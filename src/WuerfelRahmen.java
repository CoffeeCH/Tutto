...
class WuerfelRahmen {
  ...
  /**
   *  zählt die anzahl Blockierte um eins, wenn die Würfel blockiert sind.
   *  wenn anzahl Blockierten gleich gross wie die anzahl Würfel ist, wird die Methode Reset aufgerufen.
   */
  void resetNachTutto() {
    int anzB = 0;
    for (Wuerfel w : wuerfelListe) {
      if (w.istBlockiert == true) anzB++;
    }
   ...
   *  @param mx, my: die mMaus positionen.
   */
  ...
  /**
   *  Wenn würfeln vorhanden sind, wird anzNeuAngewaehlt auf eins erhöht. Die anzahl Würfeln werden gezählt.
   *  @param aaz: augen anzahl
   *  @return: die anzahl Neu angewählten Würfeln.
   */
  int getAnzahlNochNichtGezaehltenWuerfeln(int augenAnzahl) {  
    int anzNeuAngewaehlt = 0;
    for (Wuerfel w : wuerfelListe) {
      if (w.istVorhanden(augenAnzahl))anzNeuAngewaehlt++;
    }
    return anzNeuAngewaehlt;
  }
  /**
   *  Wenn würfeln gültig sind, wird anzNeuAngewaehlt auf eins erhöht. Die anzahl markierten Würfeln werden gezählt.
   *  @param aaz: augen anzahl
   *  @return: die anzahl Neu angewählten Würfeln.
   */
  int getAnzahlMarkierteWuerfel(int augenAnzahl) {  
    int anzNeuAngewaehlt = 0;
    for (Wuerfel w : wuerfelListe) {
      if (w.istGueltig(augenAnzahl))anzNeuAngewaehlt++;
    }
    return anzNeuAngewaehlt;
  }
  /**
  ...
  /**
   *  Prüft ob eine Auswahl vom Würfeln möglich ist.
   *  return: Anzahl Möglichkeite: INT
   */
  int getAnzahlMoeglicheAuswahl() {
    ...
    for (int aZ = 1; aZ <= 6; aZ++) {
      ...
      if (aZ == 1 || aZ == 5) {
        if (z >= 1)anzNeuAngewaehlt++;
      } else {
        if (z >= 3) anzNeuAngewaehlt++;
        if (z == 6) anzNeuAngewaehlt++;
      }
      }
  ...
  boolean istTutto(){
    ...
    for (int aZ = 1; aZ <= 6; aZ++) {
      if(anzNochNichtAngewaehlt > 0) return false;
      int z = getAnzahlNochNichtGezaehltenWuerfeln(aZ);
      if(z > 0)  anzNochNichtAngewaehlt += z;
    }
    return true;
  }
  boolean istAuswahlGueltig(int augenAnzahl){
    int anz = getAnzahlMarkierteWuerfel(augenAnzahl);  
    if( anz != 3 || anz != 6)return true;
    return true;
  }
}