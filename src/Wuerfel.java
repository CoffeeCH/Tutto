...
class Wuerfel {
  ...
  /**
   *  würflet die würfeln.
   *  return: zufälliger Anzahl augenzahl.
   */
  int wuerfle(boolean cheat) {
    if (istBlockiert == false) {
      ...
      if(cheat){
        augenAnzahl = 1;
        if (parent.random(2) > 1.0)augenAnzahl = 5;
      }
  ...
  /**
   *  überprüft ob der Würfel selektiert wurde.
   *  setzt die bedinungen das nur eine 5, 1 oder drillinge selektiert werden kann.
   *  @param:
   */
  void checkSelected(WuerfelRahmen callingFrame, int mxPos, int myPos) {
    if (mxPos >= xposRec && mxPos<= xposRec + breiteRec &&
      myPos >= yposRec && myPos <= yposRec + hoeheRec) {
      int anz = callingFrame.getAnzahlNochNichtGezaehltenWuerfeln(augenAnzahl);
      if (callingFrame.getAnzahlMarkierteWuerfel(augenAnzahl) >= 3 &&
          callingFrame.istAuswahlGueltig(augenAnzahl))anz=0;
  ...
  /**
   *  definiert einen Würfel als gültig, wenn er markiert aber noch nicht gezählt wurde.
   *  @param anzA : anzahl Augen 1..6
  ...
  /**
   *  setzt gezählt auf Wahr, wenn ein Würfel blockiert ist und die Augenzahl gleich gross wie die Augen anzahl.
   *  @param aZ: anzahl Augen 1..6
   */
  void markiertAlsGezaehlt(int augenZahl) {
    if (augenZahl==augenAnzahl && istBlockiert == true) istGezaehlt = true;
  }
  /**
